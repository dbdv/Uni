...
void menu(aula aulas[], int *cant){
    int selec;
    printf("Elija una operaci%cn a realizar.\n\t1-->Cargar aulas\n\t2-->Mostrar aulas\n\t3-->Imprimir ID de aulas cuyo proyector tiene control\n\t4-->Salir\n-->",162);
    scanf("%i", &selec);
    switch(selec){
...
void imprimirID(aula array[], int n){
    if(n == -1)
        printf("Final de lista");
    else{
        if(array[n].proyecAula.control)
            printf("%i", array[n].id);
        else
            imprimirID(array, n-1);
    }
}
