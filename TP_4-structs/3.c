...
void menu(aula aulas[], int *cant){
    ...
    switch(selec){
        case 1: ingresaDatos(aulas, cant);
                cargado=1;
            break;
        case 2: if(cargado) imprimirDatos(aulas, *cant);
                else{
...
void ingresaDatos(aula aulas[], int *cant){
    int aux = maxAulas-*cant;
    printf("Ingrese la cantidad de aulas que va a a%cadir\n(1-%i)-->",164,aux);
    do{
        scanf("%i", cant);
        if(*cant <= 0 || *cant > maxAulas) printf("Debe ingresar una cantidad v%clida(1-%i). Intente nuevamente.\n-->",160,aux);
    }while(*cant <= 0 || *cant > maxAulas);
    ...
    for(int i=aux; i < *cant; i++){
        ...
        printf("Ingrese el tipo de aula\0-->Laboratorio\n1-->Conferencia\n2-->Teor%ia/Pr%cctica\n-->",161,160);
        do{
